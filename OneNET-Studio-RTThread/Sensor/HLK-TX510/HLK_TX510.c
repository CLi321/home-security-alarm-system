...
void HLK_TX510_OFF(void)
{
	
	  HLK_TX510_ClearDate();
    HLK_TX510.CmdDataLen = HLK_TX510_Operate_AnswerLEN;
    HLK_TX510.CurrentMode = FACEOFF;
	
    HLK_TX510_Send_ControlCmd(HLK_TX510_Control_DisplayOFF);
    while(HLK_TX510.ifGetData == FALSE); //等待接收完应答数据

//    HLK_TX510_ClearDate();
//    HLK_TX510_Send_ControlCmd(HLK_TX510_LightOFF);
//    while(HLK_TX510.ifGetData == FALSE); //等待接收完应答数据	
	
//    HLK_TX510_ClearDate();
//    HLK_TX510_Send_ControlCmd(HLK_TX510_WhiteLightOFF);
//    while(HLK_TX510.ifGetData == FALSE); //等待接收完应答数据
	
}
void HLK_TX510_ON(void)
{
	
    HLK_TX510_ClearDate();	
    HLK_TX510.CmdDataLen = HLK_TX510_Operate_AnswerLEN;
    HLK_TX510.CurrentMode = FACEON;
	
    HLK_TX510_Send_ControlCmd(HLK_TX510_Control_DisplayON);
    while(HLK_TX510.ifGetData == FALSE); //等待接收完应答数据
	
//    HLK_TX510_ClearDate();
//    HLK_TX510_Send_ControlCmd(HLK_TX510_LightON);
//    while(HLK_TX510.ifGetData == FALSE); //等待接收完应答数据
	
//    HLK_TX510_ClearDate();
//    HLK_TX510_Send_ControlCmd(HLK_TX510_WhiteLightON);
//    while(HLK_TX510.ifGetData == FALSE); //等待接收完应答数据
...
void Dealwith_HLK_TX510(void)
{
		...
		if ( HLK_TX510.ifGetData == TRUE )
    {
				...
				{	
						...
						{
								perform_IdentifyFace_Error();
						}
//						else if(Receive_Data_TX510.Receive_BUF[6]==0x02 && Receive_Data_TX510.Receive_BUF[7]==0x12 && Receive_Data_TX510.Receive_BUF[8]==0x08) 
//						{
//								perform_IdentifyFace_Error();
//						}
				}
				...
				else if( HLK_TX510.CurrentMode == FACEOFF )
				{
						HLK_TX510.TimeCount = 0;

						if(Receive_Data_TX510.Receive_BUF[7]==0x02 && Receive_Data_TX510.Receive_BUF[8]==0xC0 && Receive_Data_TX510.Receive_BUF[9]==0x00 && Receive_Data_TX510.Receive_BUF[10]==0xC2)
						{
															
						}
				}	
				else if( HLK_TX510.CurrentMode == FACEON )
				{
						HLK_TX510.TimeCount = 0;
...
void perform_IdentifyFace_Error(void)
{
		ALARM.Trespass = 1;
}
void perform_IdentifyFace_Error2(void)
{
